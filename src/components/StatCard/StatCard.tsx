import { MaterialIcon } from '@/components/Loader/MaterialIcon/MaterialIcon'

interface StatCardProps {
  label: string
  value: number
  icon: string
  tone?: 'primary' | 'warning'
}

export function StatCard({ label, value, icon, tone = 'primary' }: StatCardProps) {
  return (
    <div className="flex items-center gap-4 rounded-lg bg-surface-container-lowest p-5 shadow-sm">
      <div
        className={`flex h-12 w-12 items-center justify-center rounded-full ${
          tone === 'warning' ? 'bg-tertiary-container/20 text-tertiary' : 'bg-primary-container/10 text-primary'
        }`}
      >
        <MaterialIcon className="text-2xl" name={icon} />
      </div>
      <div>
        <p className="font-headline text-2xl font-extrabold text-on-surface">{value}</p>
        <p className="text-xs font-semibold text-on-surface-variant">{label}</p>
      </div>
    </div>
  )
}